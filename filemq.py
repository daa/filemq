...
__version__ = '0.21'
__date__ = '2010-01-15 08:55:28 +0300'
...
class Message(object):
    ...
    """
    def __init__(self, body='', msg_id=None):
        self.body = body
        self.__inner_id = msg_id
    def __repr__(self):
    ...
    def get_inner_id(self):
        """
        return internally used id of message
        """
        return self.__inner_id
    def set_inner_id(self, msg_id):
        """
        set internally used id of message
        """
        self.__inner_id = msg_id
...
class FileMQ(object):
    ...
    def chk_spool(self):
        ...
        if not os.path.exists(self.spooldir):
            try:
                ...
                raise FileMQException(
                    'Spool directory does not exist and could not create it: %s' %
                        self.spooldir)
    ...
    def put(self, msg):
        ...
        If msg.get_inner_id() is set message file name will be formed from this id,
        otherwise from current time and random mkstemp part
        """
        try:
            self.chk_spool()
            ...
            if msg.get_inner_id() is None:
                ...
                lt = int(t)
                ft = (t - lt)*1e7
                fd, name = mkstemp(suffix=self.tmp_suffix,
                    prefix=time.strftime("%Y%m%d_%H%M%S_%.f_") % ft,
                    dir=self.spooldir)
            else:
    ...
    def clear(self):
        """
        Discard all messages from queue
        """
